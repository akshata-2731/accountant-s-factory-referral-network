...
import { getAdminData, setReferralReminder, subscribeToNewReferrals, unsubscribeFromNewReferrals, verifyAccount } from './services/mockApiService';
...
const GOOGLE_CLIENT_ID = '846999197799-pjlguh7e86r56lhdnlddvie7m4ao9fpq.apps.googleusercontent.com';
...
const App: React.FC = () => {
    ...
    const handleNewReferral = (newReferral: Referral) => {
      setNotifications(prev => {
        ...
        return [
          ...
          {
            id: newReferral.id,
            message: `New lead from ${newReferral.referrerName} for ${newReferral.clientName}.`,
            isRead: false,
          },
        ];
      });
  ...
  // This function receives user info directly from the Google login success handler in LandingPage
  ...
  const renderContent = () => {
    if (!currentUser) {
      return <LandingPage onGoogleLogin={handleGoogleLogin} authMessage={authMessage} />;
    }
  ...
  return (
    <GoogleOAuthProvider clientId={GOOGLE_CLIENT_ID}>
      <div className="min-h-screen flex flex-col bg-slate-50 text-brand-gray">
        <Header currentUser={currentUser} onLogout={handleLogout} unreadCount={unreadCount} />
